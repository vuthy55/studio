...
import { LoaderCircle, PlusCircle, MessageSquare, MapPin, ExternalLink, Compass, UserCircle, Calendar, Users as UsersIcon, LocateFixed, LocateOff, Bell, RefreshCw, ChevronRight, HelpCircle, Lock, Search, Tag, ShieldAlert } from 'lucide-react';
...
function VibeList({ vibes, parties, title, source, searchTerm, setSearchTerm }: { vibes: ClientVibe[], parties: ClientParty[], title: string, source: 'public-vibes' | 'my-vibes', searchTerm: string, setSearchTerm: (term: string) => void }) {
    ...
    const VibeLink = ({ vibe, children }: { vibe: ClientVibe, children: React.ReactNode }) => {
        const isUnderReview = vibe.status === 'under_review';
        const isAdmin = userProfile?.role === 'admin';
        
        if (isUnderReview && !isAdmin) {
            return (
                <TooltipProvider>
                    <Tooltip>
                        <TooltipTrigger asChild>
                             <div className="relative w-full opacity-50 cursor-not-allowed">{children}</div>
                        </TooltipTrigger>
                        <TooltipContent>
                            <p>This Vibe is under review and cannot be accessed.</p>
                        </TooltipContent>
                    </Tooltip>
                </TooltipProvider>
            )
        ...
        return <Link href={`/common-room/${vibe.id}?from=${source}`} className="block">{children}</Link>
    }
                ...
                <div className="text-muted-foreground text-sm text-center py-8">
                 ...
                 <div className="border rounded-lg">
                    {vibes.map((vibe, index) => {
                        ...
                        return (
                            <VibeLink vibe={vibe} key={vibe.id}>
                                <div className={cn(
                                    `flex items-start p-4 hover:bg-muted/50 transition-colors`, 
                                    index < vibes.length - 1 ? 'border-b' : '',
                                    isUnderReview && 'bg-amber-100/50 dark:bg-amber-900/20'
                                )}>
                                    <div className="flex-1 space-y-1">
                                        <div className="flex items-center gap-2">
                                            <p className="font-semibold">{vibe.topic}</p>
                                            {isUnderReview && <Badge variant="destructive"><ShieldAlert className="h-3 w-3 mr-1"/>Under Review</Badge>}
