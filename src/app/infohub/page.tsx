...
function LatestIntelDisplay({ intel, searchDate }: { intel: Partial<CountryIntel> | null, searchDate: Date | null }) {
    if (!intel || intel.finalScore === undefined) {
        return <p className="text-sm text-center text-muted-foreground py-8">Select a country to retrieve real-time information.</p>;
    }
...
function LocationIntelTab() {
    ...
    const handleCountrySelection = useCallback(async (countryCode: string) => {
        ...
        // Now automatically trigger AI intel generation
        ...
        if ((userProfile.tokenBalance || 0) < cost) {
            toast({ variant: 'destructive', title: 'Insufficient Tokens', description: `You need ${cost} tokens to generate intel.` });
            return;
        }
                 ...
                 <Tabs value={activeInfoTab} onValueChange={(v) => setActiveInfoTab(v as InfoTab)} className="w-full">
                    <TabsList className="grid w-full grid-cols-5">
                        <TabsTrigger value="latest">
                            ...
                            <AlertTriangle className="mr-2"/> Latest
                        </TabsTrigger>
                    ...
                    <TabsContent value="latest" className="mt-4">
                        <Card>
                            <CardHeader>
                                <CardTitle>Latest Intelligence for {selectedCountryName}</CardTitle>
