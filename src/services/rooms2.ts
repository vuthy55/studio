 ...
 */
export async function getAllRooms2(): Promise<ClientSyncRoom[]> {
  console.log("[SERVER ACTION] getAllRooms2 invoked.");
  try {
    ...
    for (const docSnapshot of snapshot.docs) {
        ...
        const toISO = (ts: any): string | undefined => {
            try {
                ...
                console.warn(`[getAllRooms2] Could not convert timestamp for doc ${docSnapshot.id}:`, ts);
                return undefined;
            } catch (e) {
                 console.error(`[getAllRooms2] Error converting timestamp for doc ${docSnapshot.id}:`, e);
                return undefined;
            }
        };
    ...
    console.error("[SERVER ACTION] Error in getAllRooms2:", error);
